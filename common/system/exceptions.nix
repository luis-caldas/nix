  ...
  config = {
    ...
    nixpkgs.config.packageOverrides = ogpkgs: (
      ...
      // {
        ...
        OVMFFull = ogpkgs.OVMFFull.overrideAttrs (attrs: {
            pname = attrs.pname + "-custom-logo";
            postPatch = (if (builtins.hasAttr "postPatch" attrs) then attrs.postPatch else "") + ''
              "${ogpkgs.ffmpeg}/bin/ffmpeg" -i "${my.projects.wallpapers}/papes/dpm-navy-small.png" -pix_fmt rgb24 -y -vf scale=256:-1 "./MdeModulePkg/Logo/Logo.bmp"
            '';
        });
        fprintd = mpkgs.fprintd-clients;
        # Fix Intel OCL URL
        intel-ocl = ogpkgs.intel-ocl.overrideAttrs (oldAttrs: {
          src = ogpkgs.fetchzip {
